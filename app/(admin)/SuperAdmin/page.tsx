...
const Sidebar: React.FC<SidebarProps> = ({ selectedSection, setSelectedSection, isOpen, toggleMenu }) => {
  ...
  return (
    <aside className={`fixed md:sticky top-0 h-screen bg-gradient-to-t from-gray-900 to-gray-700 p-5 flex flex-col z-50 md:w-64 ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 md:translate-x-0`}>
      <div className="flex justify-between items-center mb-8">
        <div className="text-2xl font-bold text-gradient bg-clip-text text-transparent bg-gradient-to-r from-pink-500 via-purple-600 to-indigo-600"> 
      ...
      <nav className="flex-grow overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-800">
            ...
            <li key={section}>
              <button
                ...
                className={`flex items-center space-x-2 text-lg font-medium transition duration-300 
                            ${selectedSection === section ? 'text-gradient bg-clip-text text-transparent bg-gradient-to-r from-pink-500 via-purple-600 to-indigo-600' : 'text-gray-300 hover:text-gradient hover:bg-clip-text hover:text-transparent hover:bg-gradient-to-r from-pink-500 via-purple-600 to-indigo-600'}`}
              >
      ...
      <div className="mt-6 space-y-3">
        <button className="bg-gradient-to-r from-pink-500 via-purple-600 to-indigo-600 text-white py-2 px-4 w-full rounded hover:bg-gradient-to-l hover:from-indigo-600 hover:via-purple-600 hover:to-pink-500 transition duration-300">
          Settings
        </button>
        <button className="bg-gradient-to-r from-pink-500 via-purple-600 to-indigo-600 text-white py-2 px-4 w-full rounded hover:bg-gradient-to-l hover:from-indigo-600 hover:via-purple-600 hover:to-pink-500 transition duration-300" onClick={logout}>
          Log Out
        </button>
      </div>
    </aside>
...
const Dashboard = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gray-800 text-white flex">
      ...
      <main className="flex-1 p-6 bg-gradient-to-t from-gray-800 to-gray-900 overflow-y-auto">
        {renderContent()}
      </main>
    </div>
  );
